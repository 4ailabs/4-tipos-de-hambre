...
const Quiz: React.FC = () => {
    ...
    return (
        <section className="max-w-2xl mx-auto bg-white rounded-xl shadow-lg p-6 sm:p-10 mb-12 animate-fade-in-slow transition-colors duration-300">
            <h3 className="text-2xl font-bold text-center text-orange-800 mb-6 transition-colors duration-300">Test: ¿Qué tipo de hambre predomina en ti?</h3>
            {!result ? (
                <div className="flex flex-col gap-6">
                    <div className="text-lg sm:text-xl font-semibold text-stone-800 text-center mb-2 transition-colors duration-300 animate-fade-in">
                    ...
                    <div className="flex flex-col gap-4">
                        {currentQuestion.answers.map((answer, idx) => (
                            <button
                                ...
                                className="w-full py-3 px-4 rounded-lg bg-orange-100 text-orange-900 font-bold shadow hover:bg-orange-200 transition-all duration-200 text-base sm:text-lg focus:outline-none focus:ring-2 focus:ring-orange-400 animate-fade-in"
                            >
                                {answer.text}
                            </button>
                        ))}
                    </div>
                    <div className="text-sm text-stone-500 text-center mt-2">Pregunta {currentQuestionIndex + 1} de {questions.length}</div>
                </div>
            ) : result && resultData ? (
                <div id="result-container" className="text-center animate-fade-in-slow">
                    <h3 className="text-2xl font-bold text-orange-800 mb-4">Tu Punto de Partida</h3>
                    ...
                    <p id="result-description" className="text-stone-700 mb-6 transition-colors duration-300">
